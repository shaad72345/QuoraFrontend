...
const handleAddComment = ()=>{
    const payload = {comment:AddComment}
    setShowComments(!showComments);
    fetch(`https://quoraclonebackend.onrender.com/comment/add/${props._id}`, {
      method:"POST",
      headers : {
    ...
    .then((res)=>{setShowComments(!showComments);toast(res.message) ;})
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
        ...
        fetch(`https://quoraclonebackend.onrender.com/comment/${props._id}`, {
            method : "GET",
            headers : {
              'Authorization' : `Bearer ${isLogin}`
            }
          }).then((res)=>res.json())
          .then((res)=>{setComments(res);console.log(res);setNoOfComment(res.length)})
        ...
        <>
            ...
            {/* {postdata?.map((elem) => { */}
                ...
                <Box display="flex" gap="0.5rem" alignItems="center" p="0.6rem 0.5rem">
                    ...
                    <Box>
                        <Text fontSize="m" as="b">{props.userId?.name}</Text>
                        <Text fontSize="xs">{getRelativeTime(props.createdAt)}</Text>
                    </Box>
                </Box>
        <hr />
                <Box p="0 0.5rem">
              ...
              { props.Post_Image ? <AspectRatio ratio={4 / 3} mt="0.6rem" borderTop="0.2px solid lightgray" borderBottom="0.2px solid lightgray">
                    <Center>
                        <Image
                ...
                <Box display="flex" p="0.2rem 0.5rem" alignItems="center">
                    <Box display="flex" p="0.2rem 0.5rem" alignItems="center" gap="0.8rem">
                        ...
                        <IconButton borderRadius='5rem' size='sm' isAttached variant='outline' onClick={toggleComments} icon={<FaRegComment />} />
