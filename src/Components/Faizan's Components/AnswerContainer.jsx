    ...
    return (
        ...
        <>
            ...
            {/* {postdata?.map((elem) => { */}
                <>
                    {/* <h1>{elem.des}</h1> */}
                    <Box bg="white" margin='3px' border="0.1px solid lightgray" borderRadius='10px'>
                <Box display="flex" gap="0.3rem" alignItems="center" p="0.2rem 0.4rem">
                <Box>
                        <Image
                    ...
                    <Box>
                        <Text fontSize="xs" as="b">{prop?.userId.name}</Text>
                        <Text fontSize="xs">{getRelativeTime(prop?.createdAt)}</Text>
                    </Box>
                </Box>
        <hr />
